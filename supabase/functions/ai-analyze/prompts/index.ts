...
import { DEFENSE_PROMPT, DEFENSE_ANALYSIS_TYPE } from './defense.ts';
import { PROSECUTION_PROMPT, PROSECUTION_ANALYSIS_TYPE } from './prosecution.ts';
import { JUDGE_PROMPT, JUDGE_ANALYSIS_TYPE } from './judge.ts';
import { AGGREGATOR_PROMPT, AGGREGATOR_ANALYSIS_TYPE } from './aggregator.ts';
import { EVIDENCE_PROMPT, EVIDENCE_ANALYSIS_TYPE } from './evidence.ts';
import { QUALIFICATION_PROMPT, QUALIFICATION_ANALYSIS_TYPE } from './qualification.ts';
import { PROCEDURAL_PROMPT, PROCEDURAL_ANALYSIS_TYPE } from './procedural.ts';
import { SUBSTANTIVE_PROMPT, SUBSTANTIVE_ANALYSIS_TYPE } from './substantive.ts';
import { RIGHTS_PROMPT, RIGHTS_ANALYSIS_TYPE } from './rights.ts';
...
export const PROMPT_REGISTRY: Record<AnalysisType, string> = {
 ...
 */
export function formatPreviousAnalyses(analyses: Array<{ role: string; analysis: string }>): string {
  if (!analyses.length) return '';
  
  return analyses.map(a => {
    const label = ANALYSIS_TYPE_LABELS[a.role as AnalysisType] || a.role;
    return `### ${label}\n${a.analysis}`;
  }).join('\n\n---\n\n');
}
